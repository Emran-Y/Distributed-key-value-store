...
func main() {
	for {
		...
		if err != nil {
			fmt.Printf("client2 => Error connecting to server: %v\n", err)
			return
		}
		defer connection.Close() // Ensure the connection is closed when the client exits

		fmt.Println("client2 => Connected to server. Enter commands:")
		...
		for {
			...
			if err != nil {
				fmt.Printf("client2 => Error sending data: %v\n", err)
				break
			}
			...
			if err != nil {
				fmt.Printf("client2 => Server disconnected: %v\n", err)
				break
			}
			fmt.Println("client2 => Response:", strings.TrimSpace(response))
		}

		// Attempt to reconnect to the server
		fmt.Println("client2 => Reconnecting...")
	}
}
